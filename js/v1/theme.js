class ThemeEngine {
  ...
  // Apply a theme based on a base hex color (v1 behavior)
  apply(baseHex) {
    const { h, s, l } = this.hexToHSL(baseHex);

    const theme = {
      // Brand palette (derived from base hex)
      "--brand": baseHex,
      ...
      // v1 accent colors (derived, subtle -- NOT gold)
  ...
  // Apply static layout variables (v1 behavior)
  applyDefaults() {
    const defaults = {
      "--layout-bg": "#F5F6F7",
  ...
  // Utility: clamp HSL values
  clamp(value) {
    return Math.max(0, Math.min(100, value));
  }

  // Convert hex → HSL (v1 original code)
  hexToHSL(H) {
    let r = 0, g = 0, b = 0;
  ...
  // Convert HSL → hex (v1 original code)
  hslToHex(h, s, l) {
    s /= 100;
    l /= 100;
